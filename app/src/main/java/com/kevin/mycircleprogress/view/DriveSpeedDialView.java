...
public class DriveSpeedDialView extends View
{
	...
	public DriveSpeedDialView(Context context, AttributeSet attrs)
	{
		this(context, attrs, R.attr.DriveScoreCircleProgressStyle);
	}

	public DriveSpeedDialView(Context context, AttributeSet attrs, int defStyleAttr)
	{
		...
		animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
		{
			...
			{
				DriveSpeedDialView.this.currentSpeed = (float) animation.getAnimatedValue();
				postInvalidate();
			}
		});
		outRect = new RectF();
		...
		background = getResources().getDrawable(R.drawable.speed_bg);
	...
	private void initAttribute(TypedArray attributes)
	{
		...
		unit_text_MarginBottom = Utils.dp2px(getResources(), 130f);
	...
	private void initPaint()
	{
		...
		scaleCirclePaint.setStrokeWidth(Utils.dp2px(getResources(), 2f));
		...
		scaleCircleBluePaint.setStrokeWidth(Utils.dp2px(getResources(), 2f));
		...
		outProgressPaint.setStrokeWidth(Utils.dp2px(getResources(), 4));
	...
	protected void onDraw(Canvas canvas)
	{
		...
		background.setBounds(backgroundPadding,backgroundPadding,getWidth()-backgroundPadding,getHeight()-backgroundPadding);
		...
		canvas.drawText(driveDuration, cx, getHeight() - time_text_MarginBottom, timeTextPaint);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh)
	{
		...
		scaleCircleRadius = outCircleRadius - Utils.dp2px(getResources(), 7)-getPaddingLeft();

		currentSpeedLineLength = Utils.dp2px(getResources(), 20) + getPaddingLeft();
	...
	private void drawClockScale(Canvas canvas, float radius)
	{
		...
		// 计算12点处的结束坐标
		float stopX = startX;
		int stopY1 = (int) (startY + Utils.dp2px(getResources(), 5f));// 整点处的线长度为30
		// 计算画布每次旋转的角度
